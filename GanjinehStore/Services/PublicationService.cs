...
namespace GanjinehStore.Services
{
    public class PublicationService : Service<Publication>, IPublicationService
    {
        ...
        public ItemPagingViewModel<Publication> GetPaginatedPublications(string publisherName = null,
            int pageNumber = 1,
            int pageSize = 32)
        {
            ...
            var publicationsCount = PublicationsCount(publisherName);

            var pagesCount = (int)Math.Ceiling(Decimal.Divide(publicationsCount, pageSize));

            return new ItemPagingViewModel<Publication>
            {
                Items = publications.OrderByDescending(b => b.Id).Skip(skip).Take(take).ToList(),
